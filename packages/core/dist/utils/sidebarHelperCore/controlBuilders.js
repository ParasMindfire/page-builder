...
export function createControl(label, id, type, value, controlsContainer, attributes = {}) {
...
function convertUnit(value, from, to, parentRef) {
    ...
    const percentRef = parentRef !== null && parentRef !== void 0 ? parentRef : window.innerWidth;
...
export function createSpacingControl(label, id, mode, allValue, allUnit, sides, controlsContainer, attributes = {}) {
    const { min = 0, max = 1000 } = attributes;
    const unitOpts = (sel) => ['px', 'rem', 'vh', '%']
...
export function createSelectControl(label, id, currentValue, options, controlsContainer) {
    const wrapper = document.createElement('div');
    wrapper.classList.add('control-wrapper');
