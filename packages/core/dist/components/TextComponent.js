...
export class TextComponent {
    ...
    static restore(container) {
        ...
        const textSpan = closestTextComponent.querySelector('.component-text-content');
        textSpan.addEventListener('click', (event) => {
            event.stopPropagation();
            const parentHeader = textSpan.closest('.text-component');
            if (parentHeader) {
        ...
        if (closestTextComponent && textSpan) {
            ...
            if (attributeKey) {
                const attribute = TextComponent.textAttributeConfig.find(attr => attr.key === attributeKey);
                if (attribute) {
                    const controlsElement = closestTextComponent.querySelector('.component-controls');
                    const labelElement = closestTextComponent.querySelector('.component-label');
