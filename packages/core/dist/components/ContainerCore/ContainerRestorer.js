...
export function restoreResizer(element) {
    var _a;
    (_a = element.querySelector('.resizers')) === null || _a === void 0 ? void 0 : _a.remove();
...
export function restoreContainer(container, editable) {
    ...
    container.querySelectorAll('.editable-component').forEach(child => {
        ...
        if (editable !== false) {
            ...
            childElement.addEventListener('mouseenter', (event) => showLabel(event, childElement));
            childElement.addEventListener('mouseleave', (event) => hideLabel(event, childElement));
        }
        else {
