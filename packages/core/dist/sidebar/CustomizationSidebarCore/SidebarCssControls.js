...
export function populateCssControls(component, controlsContainer, addListenersFn) {
    var _a;
    ...
    SidebarUtils.createSelectControl('Display', 'display', displayValue, ['block', 'inline', 'inline-block', 'flex', 'grid', 'none'], controlsContainer);
    /* Show flex sub-controls only when the current display is flex */
    if (styles.display === 'flex' || component.style.display === 'flex') {
        SidebarUtils.createSelectControl('Flex Direction', 'flex-direction', styles.flexDirection || 'row', ['row', 'row-reverse', 'column', 'column-reverse'], controlsContainer);
        SidebarUtils.createSelectControl('Align Items', 'align-items', styles.alignItems || 'stretch', ['stretch', 'flex-start', 'flex-end', 'center', 'baseline'], controlsContainer);
    ...
    if (isCanvas) {
        ...
        SidebarUtils.createControl('Width', 'width', 'number', component.offsetWidth, controlsContainer, { min: 300, max: 2000, unit: 'px' });
        SidebarUtils.createControl('Min Height', 'min-height', 'number', parseInt(styles.minHeight) || 100, controlsContainer, { min: 0, max: 2000, unit: 'px' });
        SidebarUtils.createControl('Margin', 'margin', 'number', parseInt(styles.margin) || 0, controlsContainer, { min: 0, max: 100, unit: 'px' });
    }
    if (!isCanvas) {
        ...
        SidebarUtils.createControl('Width', 'width', 'number', wStyle.value, controlsContainer, { min: 0, unit: wStyle.unit, parentRef: parentW });
        if (isInline)
        ...
        SidebarUtils.createControl('Height', 'height', 'number', hStyle.value, controlsContainer, { min: 0, unit: hStyle.unit, parentRef: parentH });
        if (isInline)
    ...
    SidebarUtils.createControl('Background Color', 'background-color', 'color', styles.backgroundColor, controlsContainer);
    SidebarUtils.createSelectControl('Text Alignment', 'alignment', styles.textAlign, ['left', 'center', 'right'], controlsContainer);
    ...
    SidebarUtils.createControl('Font Size', 'font-size', 'number', parseInt(styles.fontSize) || 16, controlsContainer, { min: 0, max: 100, unit: 'px' });
    SidebarUtils.createSelectControl('Font Weight', 'font-weight', styles.fontWeight, [
        'normal',
        'bold',
        'bolder',
        'lighter',
    ...
    SidebarUtils.createControl('Text Color', 'text-color', 'color', styles.color || '#000000', controlsContainer);
    SidebarUtils.createControl('Border Width', 'border-width', 'number', parseInt(styles.borderWidth) || 0, controlsContainer, { min: 0, max: 20, unit: 'px' });
    ...
    SidebarUtils.createControl('Border Color', 'border-color', 'color', styles.borderColor || '#000000', controlsContainer);
