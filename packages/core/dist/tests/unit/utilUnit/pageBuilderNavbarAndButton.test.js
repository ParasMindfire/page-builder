...
jest.mock('../../../utils/utilityFunctions', () => {
    ...
    return Object.assign(Object.assign({}, real), { 
        ...
        showNotification: jest.fn().mockImplementation(real.showNotification), showDialogBox: jest.fn().mockImplementation(real.showDialogBox) });
});
...
// Imports
// ---------------------------------------------------------------------------
import { setupSaveButton, setupResetButton, setupViewButton, setupPreviewModeButtons, setupUndoRedoButtons, } from '../../../utils/pageBuilderButtonSetup.js';
import { createHeaderIfNeeded } from '../../../utils/pageBuilderNavbarSetup.js';
import { Canvas } from '../../../canvas/Canvas.js';
import { createFullScreenPreviewModal } from '../../../utils/previewModalBuilder.js';
import { createNavbar } from '../../../navbar/CreateNavbar.js';
// Real implementations (bypassing the partial mock above)
import { showNotification as realShowNotification, showDialogBox as realShowDialogBox, syntaxHighlightHTML, syntaxHighlightCSS, debounce, } from '../../../utils/utilityFunctions.js';
// Mocked versions used by button/navbar setup modules
import { showNotification as mockShowNotification, showDialogBox as mockShowDialogBox, } from '../../../utils/utilityFunctions.js';
// ---------------------------------------------------------------------------
// DOM helpers
// ---------------------------------------------------------------------------
function addBtn(id) {
...
describe('setupSaveButton', () => {
    it('does not throw when #save-btn is absent', () => {
        expect(() => setupSaveButton({})).not.toThrow();
    });
    ...
    it('calls jsonStorage.save with the canvas state on click', () => {
        addBtn('save-btn');
        Canvas.getState.mockReturnValue([{ id: 'c1' }]);
...
describe('setupResetButton', () => {
    it('does not throw when #reset-btn is absent', () => {
        expect(() => setupResetButton({})).not.toThrow();
    });
    ...
    it('passes a non-empty confirmation message to showDialogBox', () => {
        ...
        setupResetButton({});
        document.getElementById('reset-btn').click();
        const [message] = mockShowDialogBox.mock.calls[0];
    ...
    it('calls storage.remove when onConfirm is invoked', () => {
        ...
        document.getElementById('reset-btn').click();
        const [, onConfirm] = mockShowDialogBox.mock.calls[0];
        onConfirm();
    ...
    it('does NOT call storage.remove when onCancel is invoked', () => {
        ...
        setupResetButton(storage);
        document.getElementById('reset-btn').click();
        const [, , onCancel] = mockShowDialogBox.mock.calls[0];
        onCancel();
...
describe('setupViewButton', () => {
    it('does not throw when #view-btn is absent', () => {
        expect(() => setupViewButton({}, 'grid')).not.toThrow();
    });
    ...
    it('calls createFullScreenPreviewModal with the generated HTML and layoutMode', () => {
        ...
        document.getElementById('view-btn').click();
        expect(createFullScreenPreviewModal).toHaveBeenCalledWith('<html/>', 'absolute');
    });
...
describe('setupPreviewModeButtons', () => {
    function addPreviewBtns() {
        ['preview-desktop', 'preview-tablet', 'preview-mobile'].forEach(id => addBtn(id));
    }
    it('does not throw when none of the preview buttons exist', () => {
        const panel = { setPreviewMode: jest.fn() };
...
describe('createHeaderIfNeeded', () => {
    ...
    it('creates a <header> element with id "page-builder-header"', () => {
        ...
        expect(document.getElementById('page-builder-header').tagName).toBe('HEADER');
    });
    ...
    it('appends the nav element returned by createNavbar inside the header', () => {
        ...
        createNavbar.mockReturnValue(fakeNav);
        makeApp();
        createHeaderIfNeeded(true);
        const header = document.getElementById('page-builder-header');
