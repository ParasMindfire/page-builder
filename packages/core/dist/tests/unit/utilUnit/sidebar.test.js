...
// ---------------------------------------------------------------------------
import { createPageSizeSelect, PAGE_SIZES, PAGE_SIZES_OPTIONS, createControl, createSelectControl, rgbToHex, createAttributeControls, populateModalButton, populateRowVisibilityControls, } from '../../../utils/sidebarHelperCore.js';
...
function makeAttribute(overrides = {}) {
    return Object.assign({ id: 'attr-id', key: 'test-key', title: 'Test Title', type: 'Input', input_type: 'text', value: '', execute_order: 0, default_value: '', editable: true }, overrides);
}
beforeEach(() => {
    jest.clearAllMocks();
...
describe('createControl', () => {
    ...
    it('renders a label with the correct text', () => {
        ...
        const label = container.querySelector('label');
        expect(label === null || label === void 0 ? void 0 : label.textContent).toContain('Width');
    });
...
describe('createSelectControl', () => {
    ...
    it('renders a label with the provided text', () => {
        ...
        expect((_a = container.querySelector('label')) === null || _a === void 0 ? void 0 : _a.textContent).toContain('Align');
    });
    ...
    it('handles an empty options array without throwing', () => {
        const container = makeContainer();
        expect(() => createSelectControl('Empty', 'empty', '', [], container)).not.toThrow();
    });
});
// ===========================================================================
...
describe('createAttributeControls', () => {
    ...
    it('renders a label with the attribute title', () => {
        var _a;
        const panel = makeContainer();
        createAttributeControls(makeAttribute({ title: 'My Field' }), panel, jest.fn());
        expect((_a = panel.querySelector('label')) === null || _a === void 0 ? void 0 : _a.textContent).toContain('My Field');
    });
    describe('text input type (default)', () => {
        it('renders a text input', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ input_type: 'text', key: 'txt' }), panel, jest.fn());
            const input = document.getElementById('txt');
            expect(input.type).toBe('text');
        });
        it('sets the placeholder from the title', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ input_type: 'text', key: 'ph', title: 'My Label' }), panel, jest.fn());
            const input = document.getElementById('ph');
            expect(input.placeholder).toContain('my label');
        });
        it('sets the default value on the input', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ key: 'dv', default_value: 'hello' }), panel, jest.fn());
    ...
    describe('number input type', () => {
        it('renders a number input', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ input_type: 'number', key: 'num' }), panel, jest.fn());
    ...
    describe('checkbox input type', () => {
        it('renders a checkbox input', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ input_type: 'checkbox', key: 'chk' }), panel, jest.fn());
            const input = document.getElementById('chk');
            expect(input.type).toBe('checkbox');
        });
        it('is checked when default_value is "true"', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({
                input_type: 'checkbox',
                key: 'chk2',
                default_value: 'true',
            }), panel, jest.fn());
    ...
    describe('editable = true (default)', () => {
        it('does NOT render the read-only badge', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ editable: true }), panel, jest.fn());
    ...
    describe('editable = false', () => {
        it('renders the read-only badge', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ editable: false }), panel, jest.fn());
            expect(panel.querySelector('.readonly-badge')).not.toBeNull();
        });
        it('does NOT render the event-trigger selector', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ key: 'ro1', editable: false }), panel, jest.fn());
            expect(document.getElementById('event-selector-ro1')).toBeNull();
        });
        it('renders the input as disabled', () => {
            const panel = makeContainer();
            createAttributeControls(makeAttribute({ key: 'ro2', editable: false }), panel, jest.fn());
...
describe('populateModalButton', () => {
    ...
    it('clicking Set Attribute calls handleComponentClick for a text-component', () => __awaiter(void 0, void 0, void 0, function* () {
        const panel = makeContainer();
        const comp = makeComponent(['text-component']);
        populateModalButton(comp, panel, true);
        yield panel
            .querySelector('.set-attribute-button')
            .click();
        yield Promise.resolve();
        expect(handleComponentClick).toHaveBeenCalled();
    }));
    it('clicking Set Attribute calls handleComponentClick for a header-component', () => __awaiter(void 0, void 0, void 0, function* () {
        const panel = makeContainer();
...
describe('populateRowVisibilityControls', () => {
    ...
    it('renders existing rules stored on the row element', () => {
        ...
        populateRowVisibilityControls(row, INPUTS);
        expect(document.getElementById('rules-list').querySelectorAll('.rule-item')).toHaveLength(1);
    });
...
describe('SidebarUtils delegation', () => {
    ...
    it('createSelectControl delegates and produces the expected select', () => {
        const container = makeContainer();
        SidebarUtils.createSelectControl('Font', 'font', 'bold', ['normal', 'bold'], container);
    ...
    it('createAttributeControls delegates and renders the container', () => {
        const panel = makeContainer();
        SidebarUtils.createAttributeControls(makeAttribute({ key: 'su1' }), panel, jest.fn());
