...
import { SeedFormulaValues, UpdateInputValues, UpdateCellContent, } from '../../components/TableCore.js';
import { EvaluateRowVisibility, EvaluateRule, } from '../../components/TableCore.js';
...
import { GetDefaultValuesOfInput, Restore } from '../../components/TableCore.js';
// ── Mocks ────────────────────────────────────────────────────────────────────
jest.mock('../../canvas/Canvas', () => ({
    Canvas: {
        generateUniqueClass: jest.fn((prefix) => `${prefix}-test-id`),
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function makeTable(rows = 2, cols = 2, isPreview = false) {
...
describe('TableComponent', () => {
    ...
    describe('create()', () => {
        ...
        it('should create the correct number of cells per row', () => {
            ...
            const firstRow = el.querySelector('.table-row');
        ...
        it('Add Row button click should call AddRows', () => {
            ...
            const btn = el.querySelector('.add-multiple-rows-button');
            const input = el.querySelector('.row-count-input');
            input.value = '2';
...
describe('CreateTableCell', () => {
    ...
    it('add button click should call AddCellToRow (appends a new cell)', () => {
        ...
        const cell = row.querySelector('.table-cell');
        const addBtn = cell.querySelector('.add-cell-button');
        addBtn.click();
        expect(row.querySelectorAll('.table-cell').length).toBe(3);
    });
    it('delete button click should remove the cell', () => {
        ...
        const deleteBtn = cells[0].querySelector('.delete-cell-button');
        deleteBtn.click();
...
describe('DeleteCell', () => {
    ...
    it('should remove the whole row when deleting the last cell (multiple rows)', () => {
        ...
        const cell = row1.querySelector('.table-cell');
        DeleteCell(cell);
...
describe('AddRows', () => {
    beforeEach(() => {
        document.body.innerHTML = '';
        Canvas.historyManager.captureState.mockClear();
    });
    ...
    it('should default to 1 cell when wrapper is empty', () => {
        ...
        document.body.appendChild(wrapper);
        AddRows(wrapper, 'tbl1', 1);
        const row = wrapper.querySelector('.table-row');
        expect(row.querySelectorAll('.table-cell').length).toBe(1);
    });
});
// ─────────────────────────────────────────────────────────────────────────────
...
describe('SeedFormulaValues', () => {
    beforeEach(() => {
        document.body.innerHTML = '';
        Canvas.dispatchDesignChange.mockClear();
    });
...
describe('EvaluateRowVisibility', () => {
    beforeEach(() => (document.body.innerHTML = ''));
    function makeRow(rules) {
        const wrapper = document.createElement('div');
        wrapper.classList.add('table-component');
        const row = document.createElement('div');
        row.classList.add('table-row');
        if (rules)
            row.setAttribute('data-visibility-rules', JSON.stringify(rules));
        wrapper.appendChild(row);
        document.body.appendChild(wrapper);
        return row;
    }
    ...
    it('should scope to provided table element', () => {
        ...
        row1.setAttribute('data-visibility-rules', JSON.stringify([
        ...
        row2.setAttribute('data-visibility-rules', JSON.stringify([
            { inputKey: 'x', operator: 'equals', value: '1', action: 'hide' },
        ]));
...
describe('Restore', () => {
    ...
    function buildTable(rows = 1, cols = 2, withButtons = true) {
        const tc = new TableComponent();
    ...
    it('should log error if no table-wrapper found', () => {
        ...
        Restore(empty, true, []);
        expect(consoleSpy).toHaveBeenCalledWith('No table wrapper found in container');
    });
    ...
    it('should remove contenteditable from cells when editable=false', () => {
        const el = buildTable();
        Restore(el, false, []);
        const content = el.querySelector('.table-cell-content');
