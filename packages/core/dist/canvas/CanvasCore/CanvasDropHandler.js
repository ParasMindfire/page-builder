...
export class CanvasDropHandler {
    static onDrop(event) {
        ...
        const { canvasElement, layoutMode, components, historyManager } = CanvasSharedState;
        ...
        let customSettings = (_b = event.dataTransfer) === null || _b === void 0 ? void 0 : _b.getData('custom-settings');
        if (!componentType)
            return;
        /** If no custom settings in the transfer, fall back to globally registered config */
        if (!customSettings || customSettings.trim() === '') {
            const draggableElement = document.querySelector(`[data-component="${componentType}"]`);
            if (draggableElement) {
                const customComponents = window.customComponents;
                if ((_c = customComponents === null || customComponents === void 0 ? void 0 : customComponents[componentType]) === null || _c === void 0 ? void 0 : _c.settings) {
                    customSettings = JSON.stringify(customComponents[componentType].settings);
                }
            }
        }
        /** Grid snapping: resolve the drop to the nearest grid-corner position */
        const { gridX, gridY } = CanvasSharedState.gridManager.mousePositionAtGridCorner(event, canvasElement);
        ...
        if (layoutMode === 'absolute' &&
            ...
            const innerContentRightX = canvasElement.offsetWidth - paddingRight - MIN_COMPONENT_WIDTH;
            if (gridX < paddingLeft ||
                gridY < paddingTop ||
                ...
                setTimeout(() => canvasElement.classList.remove('container-highlight'), 300);
                return;
            }
        }
        const component = CanvasComponentFactory.createComponent(componentType, customSettings);
        if (component && CanvasSharedState.editable !== false) {
            const uniqueClass = CanvasComponentFactory.generateUniqueClass(componentType);
