...
import { ButtonComponent, HeaderComponent, ImageComponent, VideoComponent, TextComponent, ContainerComponent, TwoColumnContainer, ThreeColumnContainer, TableComponent, LinkComponent, } from '../../components.js';
...
export class CanvasComponentFactory {
    ...
    static get factoryMap() {
        const { headerAttributeConfig, ImageAttributeConfig, tableAttributeConfig, textAttributeConfig, historyManager, } = CanvasSharedState;
        return {
            button: () => new ButtonComponent().create(),
            header: () => new HeaderComponent().create(1, 'Header', headerAttributeConfig),
            image: () => new ImageComponent().create(undefined, ImageAttributeConfig),
            video: () => new VideoComponent(() => historyManager.captureState()).create(),
            table: () => new TableComponent().create(2, 2, undefined, tableAttributeConfig),
    ...
    static createComponent(type, customSettings = null, props) {
        ...
        if (factoryFn) {
            ...
            const tagNameElement = document.querySelector(`[data-component='${type}']`);
            const tagName = tagNameElement === null || tagNameElement === void 0 ? void 0 : tagNameElement.getAttribute('data-tag-name');
            if (tagName) {
        ...
        if (element && editable !== false) {
            ...
            const resizeObserver = new ResizeObserver(() => {
                if (layoutMode === 'absolute' &&
                    CanvasSharedState.canvasElement.classList.contains('preview-printable')) {
                    const style = window.getComputedStyle(CanvasSharedState.canvasElement);
                    ...
                    if (elementLeft + elementWidth > maxCanvasWidth - paddingRight) {
                        const maxAllowedWidth = maxCanvasWidth - paddingLeft - paddingRight - elementLeft;
    ...
    static generateUniqueClass(type, isContainerComponent = false, containerClass = null) {
        ...
        if (isContainerComponent && containerClass) {
            ...
            let containerElement = (_a = CanvasSharedState.components.find(c => c.classList.contains(containerClass))) !== null && _a !== void 0 ? _a : null;
            if (!containerElement) {
