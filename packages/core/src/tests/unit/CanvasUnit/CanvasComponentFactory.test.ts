import { CanvasComponentFactory } from '../../../canvas/CanvasCore/CanvasComponentFactory';
...
jest.mock('../../../components', () => ({
  ...
  ContainerComponent: jest
    .fn()
    .mockImplementation(() => ({
      create: () => document.createElement('div'),
    })),
  TwoColumnContainer: jest
    .fn()
    .mockImplementation(() => ({
      create: () => document.createElement('div'),
    })),
  ThreeColumnContainer: jest
    .fn()
    .mockImplementation(() => ({
      create: () => document.createElement('div'),
    })),
...
describe('CanvasComponentFactory', () => {
  ...
  describe('createComponent()', () => {
    it('should create a built-in component (button)', () => {
      ...
      expect(element?.classList.contains('editable-component')).toBe(true);
    });
    ...
    it('should attach control buttons if editable is true', () => {
      ...
      expect(
        CanvasSharedState.controlsManager.addControlButtons
      ).toHaveBeenCalled();
    });

    it('should not add editable classes if editable is false', () => {
      (CanvasSharedState as any).editable = false;
      const element = CanvasComponentFactory.createComponent('button');
      expect(element?.classList.contains('editable-component')).toBe(false);
      (CanvasSharedState as any).editable = true; // reset
    });
    ...
    it('should handle custom web-components using data-tag-name', () => {
      ...
      expect(element?.tagName.toLowerCase()).toBe('my-custom-element');
      expect(element?.classList.contains('custom-component')).toBe(true);
    });
  });
  ...
  describe('generateUniqueClass()', () => {
    ...
    it('should handle container-scoped unique IDs', () => {
      ...
      const id = CanvasComponentFactory.generateUniqueClass(
        'inner',
        true,
        'cont1'
      );
